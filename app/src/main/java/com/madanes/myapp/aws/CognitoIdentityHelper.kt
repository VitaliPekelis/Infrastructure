package com.madanes.myapp.aws

import android.content.Context
import com.amazonaws.mobileconnectors.cognitoidentityprovider.CognitoUserPool
import com.amazonaws.regions.Regions
import com.madanes.myapp.R
...
class CognitoIdentityHelper private constructor(context: Context)
{
    ...
    val signUpFieldsC2O: Map<String, String>
    val signUpFieldsO2C: Map<String, String>
    ...
    init
    {
        ...
        mClientSecret  = resources.getString(R.string.clientSecret)
        userPool = CognitoUserPool(context, userPoolId, clientId, mClientSecret, cognitoRegion)

/**
 *  sub	                    string	        Subject - Identifier for the End-User at the Issuer.
 *
 *  given_name	            string	        Given name(s) or first name(s) of the End-User. Note that in some cultures, people can have multiple given names;
 ...
 *
 *  family_name	            string	        Surname(s) or last name(s) of the End-User. Note that in some cultures, people can have multiple family names or no family name;
 *                                          all can be present, with the names being separated by space characters.
 *
 *  middle_name	            string	        Middle name(s) of the End-User. Note that in some cultures, people can have multiple middle names;
 *                                          all can be present, with the names being separated by space characters.
 ...
 *                                          The RP MUST NOT rely upon this value being unique, as discussed in Section 5.7.
 *
 *  email_verified	        boolean	        True if the End-User's e-mail address has been verified; otherwise false. When this Claim Value is true,
 ...
 *                                          separated by a dash. For example, en-US or fr-CA. As a compatibility note, some implementations have used an underscore as the separator rather than a dash, for example, en_US; Relying Parties MAY choose to accept this locale syntax as well.
 *
 *  phone_number	        string	        End-User's preferred telephone number. E.164 is RECOMMENDED as the format of this Claim, for example,
 *                                          +1 (425) 555-1212 or +56 (2) 687 2400. If the phone number contains an extension, it is RECOMMENDED that the extension be represented using the RFC 3966 [RFC3966] extension syntax, for example,
 *                                          +1 (604) 555-1234;ext=5678.
 *
 *  phone_number_verified	boolean	        True if the End-User's phone number has been verified; otherwise false. When this Claim Value is true,
 ...
 *
 *  address	                JSON object	    End-User's preferred postal address. The value of the address member is a JSON [RFC4627] structure containing some or all of the members defined in Section 5.1.1. on http://openid.net/specs/openid-connect-core-1_0.html#AddressClaim
 *
 ...
 */
        ...
        signUpFieldsC2O = mapOf(
                "Given name" to "given_name",
                "Family name" to "family_name",
                "Nick name" to "nickname",
                PHONE_NUMBER to "phone_number",
                "Phone number verified" to "phone_number_verified", //Boolean
                "Email verified" to "email_verified",
                EMAIL to "email",
                "Middle name" to "middle_name",
                "Address" to "address")

        signUpFieldsO2C = mapOf(
                "given_name" to "Given name",
                "family_name" to "Family name",
                "nickname" to "Nick name",
                "phone_number" to PHONE_NUMBER,
                "phone_number_verified" to "Phone number verified", //Boolean
